...
function Destinations() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const response = await fetch("/api/destination"); // Adjust URL as needed
        const data = await response.json();
        setdestinationsPosts(data); // Assuming the backend sends an array of posts
      } catch (error) {
  ...
  const sortedPosts = sortPosts([...destinationsPosts], sortCriteria);
