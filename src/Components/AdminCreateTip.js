...
function AdminCreateTip() {
  ...
  const fetchPosts = async () => {
    try {
      const response = await axios.get("/api/ti");
  ...
  const handlePostSelectChange = async (e) => {
    ...
    try {
      const { data } = await axios.get(`/api/ti/${postId}`);
      setTitle(data.title);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (selectedPostId) {
        await axios.put(`/api/ti/${selectedPostId}`, postData);
      } else {
        await axios.post("/api/ti", postData);
      }
      fetchPosts();
      clearForm();
    } catch (error) {
...
try {
  await axios.delete(`/api/ti/${selectedPostId}`);
  fetchPosts();
  clearForm();
} catch (error) {
...
return (
<div>
<h1>{selectedPostId ? "Edit Tip Post" : "Create New Tip Post"}</h1>
